...
Give NGMixer your material and mixing instructions. It prepares the project, measures levels and loudness, adjusts tracks and effects, and renders audio for you to hear. Give it feedback and keep refining the mix in the same project.
...
| --- | --- |
| [REAPER](https://www.reaper.fm/download.php) | 7.x; launch it at least once after installation |
| Node.js | `>=22.19 <23` |
...
### 3. Provide your material and start mixing

Visit the WebUI address shown in the terminal, which defaults to `http://127.0.0.1:3080`. Provide audio, a REAPER project, or a local path to your material in the conversation, and describe the result you want. For example:

> Create a project from the vocals and backing track in this folder, bring the lead vocal forward, and render a WAV for me to hear.

You only need to launch REAPER yourself once after installation. NGMixer handles project preparation and script calls from there; you do not need to open a project manually or load Lua scripts through the action list.
...
If a bridge request times out, run `pnpm doctor` to check REAPER paths and local settings, then give NGMixer the error to troubleshoot the connection. If a model request fails, check the provider credentials and default model in **Settings → Models**. For a gateway, also check the API address, protocol, and model ID.
